...
def get_context(context):
	if (frappe.session.user == "Guest" or frappe.db.get_value("User", frappe.session.user, "user_type")=="Website User"):
		...
		result = []
		q = "select w.name, v.`vineyard name`, location, c.full_name, description from tabvineyards v join `tabWork Order` w join tabCustomer c on (w.client = c.name and v.name = w.vineyard) where status='Working'"
